...
public class TouchTailComponent extends AbstractVisibleComponent {
	...
	private Plane plane;
	private PImage a;
	private TouchTailScene scene;
	
	private boolean useTexture = false;
	
	public TouchTailComponent(PApplet applet,TouchTailScene scene) {
		super(applet);
		this.scene = scene;
	...
	private class DragListener implements IGestureEventListener{
		public boolean processGestureEvent(MTGestureEvent ge) {
			...
			case DragEvent.GESTURE_ENDED:{
				...
				scene.setClearColor(new MTColor(ToolsMath.getRandom(0, 255), ToolsMath.getRandom(0, 255), ToolsMath.getRandom(0, 255), 255));
			}break;
			default:
				break;
			}
	...
	private void updateColors() {
		...
		for (int g = 0; g < nGestures; g++) {
			if ((J = gestureArray[g]).exists) {
				J.setColor(new MTColor(ToolsMath.getRandom(0, 255), ToolsMath.getRandom(0, 255), ToolsMath.getRandom(0, 255), 255));
...
//					*/
	...
	private void checkDirection(TailGesture gesture) {
		if (gesture.exists) {
			int nPts = gesture.nPoints;
			int nPts1 = nPts-1;
			Vector3D path[];
